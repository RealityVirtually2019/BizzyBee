...
export default {
  ...
  methods: {
    ...
    modelLoadSetup() {
      console.log(this.$el.getElementsByClassName('gltfmodel')[0].object3D) // I'm text inside the component.
        ...
        for (var bloomBone in bloomBones) {
          if (bloomBone) {
            ...
            psys.object3D.position.set(
              bloomBones[bloomBone].position.x,
              bloomBones[bloomBone].position.y,
              bloomBones[bloomBone].position.z
            )
            if (bloomBones[bloomBone].rotation != undefined) {
              psys.object3D.rotation.set(
                bloomBones[bloomBone].rotation.x,
                bloomBones[bloomBone].rotation.y,
                bloomBones[bloomBone].rotation.z
              )
            }
